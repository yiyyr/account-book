...
- 收入分配：收入先进入未分配资金，分配页实时显示本次安排和分配后剩余金额。
- 分配参考：显示上月分配、上月支出和本月已分配，并可通过“沿用上月”自动填入本月尚未分配的差额。
- 信封概览：首页显示各信封累计余额、本月分配、本月支出和分配消耗；发生信封转账时额外显示本月净调整。
- 类目管理：支持自定义收入类目和支出类目。
- 流水回看：默认查看本月流水，可按年份、月份、类型、类目和支出信封筛选；记录按日期分组，每组显示当日收入、支出、笔数，并在同时存在收入和支出时显示净流入或净流出。
- 列表加载：流水页每次至少展示 50 条记录，并通过“显示更多”继续展开，避免长列表一次性铺满页面。
- 统计分析：按月份查看收支趋势，并在类目占比和信封消耗中同时显示金额与支出占比。
- 数据管理：支持 CSV 流水导出，以及 JSON 完整备份和恢复。
- 离线可用：构建后通过 PWA 缓存静态资源，已缓存页面可离线打开。

## 账本口径

- 未分配资金：已经记为收入、但尚未划入任何信封的资金。
- 当前余额：信封建立以来的累计余额，由累计分配、转入、转出和支出共同决定，不会按月清零。
- 本月分配：本月从未分配资金直接划入该信封的金额，不包含信封之间的转账。
- 本月支出：本月从该信封扣除的支出金额。
- 分配消耗：本月支出除以本月分配；本月没有直接分配时显示 `--`。
- 本月调整：本月转入减去本月转出，仅在该信封本月发生过转账时显示。转账只调整信封余额，不计入收入或支出。

例如，长期储蓄信封的“当前余额”可以用于查看长期累计储蓄；资金转出或从该信封发生支出后，当前余额会相应减少。
...
git commit -m "Improve envelope allocation insights"
